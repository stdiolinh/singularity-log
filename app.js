(function () {
  ...
  function relayFailureResponse(err) {
    ...
    if (err && err.code === "network") {
      return (
        "Could not reach the relay at /api/chat. Start the app from a terminal with `npm start` in the singularity-log project, then open the printed http://127.0.0.1:… address in the same browser (not a saved file:// page)."
      );
    }
